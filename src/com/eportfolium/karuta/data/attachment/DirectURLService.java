...
public class DirectURLService extends HttpServlet {
	...
	public static String hexToString(byte[] bytes) {
		final StringBuilder hexchars = new StringBuilder(bytes.length * 2);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		final String val = request.getParameter("i");
		if (val == null) {
		...
		try {
			final byte[] data = stringToHex(val.toCharArray());
			rc4 = Cipher.getInstance("RC4");
			final SecretKeySpec key = new SecretKeySpec(secretkey.getBytes(), "RC4");
			rc4.init(Cipher.DECRYPT_MODE, key);

			final byte[] ciphertext = rc4.update(data);
		...
		final String getUserRole = request.getParameter("getuserrole");
		...
		if ("unlimited".equals(splitData[4])) {
			...
			final int duration = Integer.parseInt(splitData[4]); // In hours (minimum 1h)
			long endtime = 0;
			endtime = Long.parseLong(splitData[5]);

			/// Check if link is still valid
			final long currtime = date.getTime() / 1000;
			if (currtime > endtime) {
		...
		try {
			...
			final Integer uidcheck = (Integer) session.getAttribute("uid");
			int uid = 0;
			if (uidcheck != null) {
				uid = uidcheck;
				isLogged = true;
			...
			case 4: // Just log as public (world)
				if (!isLogged) {
					...
					final String sql = "SELECT userid FROM credential WHERE login='public'";
			...
			case 3: // Create account for this person
				if ("2world".equals(showtorole)) {
					...
					final String tld = UUID.randomUUID().toString();

					email = username + "@" + domainname + "." + tld;
				} else {
					final String add = UUID.randomUUID().toString();
					email += add;
				}
			...
			case 2: // Share portfolio
				if (uid > 0) {
					...
					final String userInfo = "<users><user id='" + uid + "' /></users>";
			...
			case 1: // Temp login
				if (!isLogged) {
					...
					final String referer = request.getHeader("referer"); // Can be spoofed
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		final String level = request.getParameter("l");
		String duration = request.getParameter("d");
		final String type = request.getParameter("type");
		final String sharerole = request.getParameter("sharerole");
		final String showtorole = request.getParameter("showtorole");
		Connection c = null;
		Document doc = null;
		...
		final NodeList metadata = doc.getElementsByTagName("metadata-wad");
		String[] values = null;
		if (metadata.getLength() > 0) {
			final Node meta = metadata.item(0);
			/// Authorized role to create share requests
			final Node nodeshareroles = meta.getAttributes().getNamedItem("shareroles");
			...
			final String shareroleval = nodeshareroles.getTextContent();
			final String[] multiplex = shareroleval.split(";");
			/// Find matching line
			String find_pattern = "";
		...
		if ("unlimited".equals(duration)) {
			...
			final long endtime = current.getTime() / 1000 + durationInt * 3600; // Number of seconds
		...
		try {
			...
			final byte[] clear = rc4.update(data.getBytes());
