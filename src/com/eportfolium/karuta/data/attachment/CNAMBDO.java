...
public class CNAMBDO extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		//// Login to service
		final String body = String.format("_username=%s&_password=%s", clientid, clientsecret);

		try {
			URL urlConn = new URL(baseURL + "/login");
			...
			final BufferedReader breader = new BufferedReader(new InputStreamReader(objReturn, "UTF-8"));
			...
			final String tokenregexp = "token\":\"([^\"]*)";
			final Pattern ptoken = Pattern.compile(tokenregexp);
			final Matcher pmatcher = ptoken.matcher(logininfo.toString());
			String access_token = "";
			if (pmatcher.find()) {
			...
			final String queryURL = String.format("%s%s%s", baseURL, pathinfo, query);
