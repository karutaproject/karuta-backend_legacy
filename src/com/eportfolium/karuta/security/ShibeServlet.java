...
public class ShibeServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		try {
			...
			if (Boolean.parseBoolean(ConfigUtils.getInstance().getProperty(SHIB_AUTH))) {
				...
				final var principalHeaderName = ConfigUtils.getInstance().getProperty(SHIB_REMOTE_USER);
				if (principalHeaderName != null && !principalHeaderName.trim().isEmpty()) {
					principalRequestHeader = principalHeaderName;
				}
				final var createUserProperty = ConfigUtils.getInstance().getProperty(SHIB_CREATE_USER);
				...
				if (createUserNotExisting) {
					...
					final var fnAndLnAreSet = firstNameRequestHeader != null
							&& !firstNameRequestHeader.trim().isEmpty() && lastNameRequestHeader != null
							&& !lastNameRequestHeader.trim().isEmpty();

					if (!fnAndLnAreSet && (fullNameRequestHeader == null || fullNameRequestHeader.trim().isEmpty())) {
						throw new IllegalArgumentException(
								"The username can't be retrieved, no shibboleth attribute initialized !");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		final var remoteUser = getRemoteUser(request);
		...
		if (remoteUser != null) {
			try {
				...
				var userId = dataProvider.getUserId(connexion, remoteUser, null);
				uid = Integer.parseInt(userId);

				if (uid == 0 && createUserNotExisting) {
					logger.info("[SHIBESERV] Creating account for {}", remoteUser);
					final var mail = getShibAttribute(request, emailRequestHeader);
					if (mail == null || mail.isEmpty()) {
						logger.error(
								"L'adresse mail '{}' n'a pas de valeur valide pour l'utilisateur avec l'identifiant '{}'",
								mail, remoteUser);
						throw new IllegalAccessException(
								"Les informations utilisateurs nécessaires n'ont pas été transmises");
					}
					...
					/// Update values
					if (!useFullName) {
						final var fn = getShibAttribute(request, firstNameRequestHeader);
						final var ln = getShibAttribute(request, lastNameRequestHeader);
						...
						if (fn != null && !fn.isEmpty() && ln != null && !ln.isEmpty()) {
							...
							logger.error(
									"Le prénom '{}' et/ou le nom '{}' n'ont pas de valeur valide pour l'utilisateur avec l'identifiant '{}'",
									ln, fn, remoteUser);
							throw new IllegalAccessException(
									"Les informations utilisateurs nécessaires n'ont pas été transmises");
						}
					} else {
						final var cn = getShibAttribute(request, fullNameRequestHeader);
						if (cn == null || cn.isEmpty()) {
							logger.error(
									"Le nom d'affichage '{}' n'a pas de valeur valide pour l'utilisateur avec l'identifiant '{}'",
									cn, remoteUser);
							throw new IllegalAccessException(
									"Les informations utilisateurs nécessaires n'ont pas été transmises");
						}
						final var namefrag = cn.split(" ");
	...
	private String getShibAttribute(final HttpServletRequest request, final String attributeName) {
		var userAttr = (String) request.getAttribute(attributeName);
