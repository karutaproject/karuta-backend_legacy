...
import com.eportfolium.karuta.security.oauth.OAuth2Parse;
...
public class LTIServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		final var cookies = processLoginCookie(request);

		final Map<String, Object> payload = new HashMap<>();
		payload.put(OAUTH_MESSAGE, OAuth2Parse.getMessage(request, null));
		final var payloadStr = processRequest(request, outTrace);
		...
		try {
			...
			final var userId = getOrCreateUser(payload, cookies, connexion, outTrace);
			...
			if (!"0".equals(userId)) // FIXME: Need more checking and/or change uid String to int
			{
				...
				final var server = "https://" + request.getServerName() + request.getContextPath();
				session.invalidate();
			...
			final var contextRole = (String) payload.get("ext_sakai_role");
			...
			*/
			...
			final var link = processEncrypted(request, payload);
			...
			if (isTrace(application)) {
				...
				try {
					...
					final var sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss_S");
					final var time = sdf.format(cal.getTime());

					final var logstream = new PrintStream(logfile);
	...
	private StringBuffer buildGroupXml(String title, String role) throws Exception {
		...
		xml.append("<group id='-1'>").append("<label>").append(title).append("</label>").append("<role>").append(role)
	...
	private StringBuffer buildUserXml(Map<String, Object> payload) throws Exception {
		final var userName = (String) payload.get(BasicLTIConstants.LIS_PERSON_SOURCEDID);
	 ...
	 * @throws Exception
	 */
	private String getOrCreateGroup(Connection connexion, String groupTitle, String role, StringBuffer outTrace)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	private String getOrCreateUser(Map<String, Object> payload, Map<String, String> cookies, Connection connexion,
		...
		if ("0".equals(userId)) {
			...
			if (ltiCreateUser) {
				...
				final var famName = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_FAMILY);
				final var gibName = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_GIVEN);
	...
	private String processEncrypted(HttpServletRequest request, Map<String, Object> payload)
		...
		var link = "";
		final var encrypted_session = (String) payload.get("ext_sakai_encrypted_session");
		final var serverid = (String) payload.get("ext_sakai_serverid");
		final var sakaiserver = (String) payload.get("ext_sakai_server");
		...
		final var application = getServletConfig().getServletContext();
		final var oauth_consumer_key = (String) payload.get("oauth_consumer_key");
		final var configPrefix = "basiclti.provider." + oauth_consumer_key + ".";
		final var oauth_secret = ConfigUtils.getInstance().getRequiredProperty(configPrefix + "secret");

		/// Fetch and decode session
		final var sha1Secret = DigestUtils.sha1Hex(oauth_secret);

		var sessionid = BlowFish.decrypt(sha1Secret, encrypted_session);
	...
	private void validate(Map<String, Object> payload, ServletContext application, StringBuffer outTrace)
		...
		final var lti_version = (String) payload.get(BasicLTIConstants.LTI_VERSION);
		final var oauth_consumer_key = (String) payload.get("oauth_consumer_key");
		final var user_id = (String) payload.get(BasicLTIConstants.USER_ID);
		final var context_id = (String) payload.get(BasicLTIConstants.CONTEXT_ID);
		...
		final var configPrefix = "basiclti.provider." + oauth_consumer_key + ".";
		final var oauth_secret = ConfigUtils.getInstance().getProperty(configPrefix + "secret");
		...
		final var oam = (OAuthMessage) payload.get(OAUTH_MESSAGE);
		final OAuthValidator oav = new SimpleOAuthValidator();
		final var cons = new OAuthConsumer("about:blank#OAuth+CallBack+NotUsed", oauth_consumer_key, oauth_secret,
				null);
