...
/**
 * Class supporting lti integration Used basiclti-util-2.1.0 (from sakai 2.9.2)
 * for the oauth dependencies
 *
 * @author chmaurer
 */
public class LTIServlet extends HttpServlet {
	...
	private StringBuffer buildUserXml(Map<String, Object> payload) throws Exception {
		final String userName = (String) payload.get(BasicLTIConstants.LIS_PERSON_SOURCEDID);
		final String fname = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_GIVEN);
		final String lname = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_FAMILY);
		final String email = (String) payload.get(BasicLTIConstants.LIS_PERSON_CONTACT_EMAIL_PRIMARY);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		final Map<String, String> cookies = processLoginCookie(request);
		final ServletContext application = getServletConfig().getServletContext();

		try {
			final HTTPRequest nimbRequest = JakartaServletUtils.createHTTPRequest(request);
			final AuthorizationRequest authRequest = AuthorizationRequest.parse(nimbRequest);

			final Map<String, Object> payload = new HashMap<String, Object>();
			payload.put(OAUTH_MESSAGE, authRequest.toString());
			final Map<String, String> payloadStr = processRequest(request, outTrace);
			...
			final String userId = getOrCreateUser(payload, connexion, outTrace);

			if ("0".equals(userId)) {
				final String server = "https://" + request.getServerName() + request.getContextPath();
				session.invalidate();
			...
			} else if (ltiUserName != null && !ltiUserName.isEmpty()) {
			...
			final String contextRole = (String) payload.get("ext_sakai_role");
			...
			*/
			...
			final String link = processEncrypted(request, payload);
			...
			if (isTrace(application)) {
				...
				try {
					...
					final java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss_S");
	 ...
	 * @throws Exception
	 */
	private String getOrCreateUser(Map<String, Object> payload, Connection connexion, StringBuffer outTrace)
			throws Exception {
		String userId = "0";
		final StringBuffer userXml = buildUserXml(payload);
		...
		if (useEmail) {
			username = (String) payload.get(BasicLTIConstants.LIS_PERSON_CONTACT_EMAIL_PRIMARY);
		} else if (!StringUtils.isBlank(ltiUserName)) {
			username = (String) payload.get(ltiUserName);
		} else {
			username = (String) payload.get(BasicLTIConstants.LIS_PERSON_SOURCEDID);
		}
		...
		if ("0".equals(userId)) {
			...
			if (ltiCreateUser) {
				...
				final String famName = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_FAMILY);
				final String gibName = (String) payload.get(BasicLTIConstants.LIS_PERSON_NAME_GIVEN);
	...
	private String processEncrypted(HttpServletRequest request, Map<String, Object> payload)
			throws UnsupportedEncodingException {
		String link = "";
		final String encrypted_session = (String) payload.get("ext_sakai_encrypted_session");
		final String serverid = (String) payload.get("ext_sakai_serverid");
		final String sakaiserver = (String) payload.get("ext_sakai_server");

		if (encrypted_session == null || serverid == null || sakaiserver == null) {
			return link;
		}
		...
		final String sha1Secret = DigestUtils.sha1Hex(oauth_secret);
	...
	private Map<String, String> processLoginCookie(HttpServletRequest request) {
		final Map<String, String> processedCookies = new HashMap<>();
	 ...
	 * @throws LTIException
	 */
	private void validate(Map<String, Object> payload, ServletContext application, StringBuffer outTrace)
			throws LTIException {

		//check parameters
		final String lti_message_type = (String) payload.get(BasicLTIConstants.LTI_MESSAGE_TYPE);
		final String lti_version = (String) payload.get(BasicLTIConstants.LTI_VERSION);
		final String oauth_consumer_key = (String) payload.get("oauth_consumer_key");
		final String user_id = (String) payload.get(BasicLTIConstants.USER_ID);
		final String context_id = (String) payload.get(BasicLTIConstants.CONTEXT_ID);
		...
		final OAuthConsumer cons = new OAuthConsumer("about:blank#OAuth+CallBack+NotUsed", oauth_consumer_key,
				oauth_secret, null);
