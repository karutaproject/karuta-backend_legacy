...
public class LTIServletUtils {
	...
	protected static String buildUsername(Map<String, Object> payload) {
		final String consumer_key = (String) payload.get(OAUTH_CONSUMER_KEY);
		final String lms_user_id = (String) payload.get(BasicLTIConstants.USER_ID);
	...
	protected static StringBuffer buildUserXml(Map<String, Object> payload) throws Exception {
		final String userName = buildUsername(payload);
	...
	protected static String getOrCreateUser(Map<String, Object> payload, Connection connexion, StringBuffer outTrace)
		...
		//Check for log entry
		final String lms_user_eid = (String) payload.get(BasicLTIConstants.LIS_PERSON_SOURCEDID);
		final String lms_user_id = (String) payload.get(BasicLTIConstants.USER_ID);
		final String consumer_key = (String) payload.get(OAUTH_CONSUMER_KEY);

		final String logId = LTIUserLog.getLogEntryId(connexion, lms_user_id, lms_user_eid, userId, consumer_key,
				outTrace);
		if ("0".equals(logId)) {
			// Create log entry
			final StringBuffer logResult = LTIUserLog.createUserLogEntry(connexion, lms_user_id, lms_user_eid, userId,
	...
	protected static void handleLaunch(Map<String, Object> payload, ServletContext application,
		...
		try {
			...
			final boolean isInSiteGroup = dataProvider.isUserInGroup(connection, userId, siteGroupId);
			final boolean isInSiteRoleGroup = dataProvider.isUserInGroup(connection, userId, siteRoleGroupId);
			...
			final String redirectURL = (String) application.getAttribute("lti_redirect_location");
	...
	protected static Connection initDB(ServletContext application, StringBuffer outTrace) throws Exception {
		...
		final Document doc = documentBuilder.parse(application.getRealPath("/") + "/META-INF/context.xml");
		final NodeList res = doc.getElementsByTagName("Resource");
		final Node dbres = res.item(0);

		final Properties info = new Properties();
		final NamedNodeMap attr = dbres.getAttributes();
		String url = "";
		for (int i = 0; i < attr.getLength(); ++i) {
			final Node att = attr.item(i);
			final String name = att.getNodeName();
			final String val = att.getNodeValue();
			if ("url".equals(name)) {
				url = val;
	...
	protected static boolean isTrace(ServletContext application) {
		final String traceStr = (String) application.getAttribute("LTIServlet.trace");
	...
	protected static void loadRoleMapAttributes(ServletContext application, HttpSession session) throws Exception {
		final java.io.FileInputStream fichierSrce = new java.io.FileInputStream(
				ConfigUtils.getInstance().getConfigPath() + "roleMap.properties");
		final java.io.BufferedReader readerSrce = new java.io.BufferedReader(
	...
	protected static void oauthValidate(HttpServletRequest request, Map<String, Object> payload,
		...
		final IMSJSONRequest ijr = new IMSJSONRequest(request);
	...
	protected static String roleMapper(ServletContext application, String inputRole, StringBuffer outTrace)
		...
		final String adjustedInput = inputRole.replaceAll(" ", "_");
		final String wadRole = (String) application.getAttribute(adjustedInput);
