		...
		try {
			if (event.eventType == KEvent.EventType.LOGIN) {
				...
				while (headerNames.hasMoreElements()) {
					...
					httpHeaders.append(headerName).append(": ").append(httpServletRequest.getHeader(headerName))
							.append("\n");
				}
				...
				dataProvider.writeLog(connection, url, httpServletRequest.getMethod(), httpHeaders.toString(),
						event.inputData, event.doc.toString(), event.status);
