...
public class OAuth2 extends HttpServlet {
	...
	final Logger logger = LoggerFactory.getLogger(OAuth2.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException
	{
		...
		if( param != null && param.containsKey("code") )	// Might be a return, check state
		{
			...
			if( retstate.equals(sesstate) )
			{
				...
				String URLToken = ConfigUtils.get("URLToken");
				
				String grant_type = "authorization_code";
				String client_id = ConfigUtils.get("OAUth_client_id");
				/// Need secret to be url encoded
				String client_secret = URLEncoder.encode(ConfigUtils.get("OAuth_client_secret"), "UTF8");
				String redirect_uri = request.getRequestURL().toString();
				String code = param.get("code");
				String authdata = String.format("grant_type=%s&client_id=%s&client_secret=%s&redirect_uri=%s&code=%s",
...
//						System.out.println("================");
						...
						response.sendRedirect( ConfigUtils.get("ui_redirect_location") );
...
//						System.out.println("Code: ("+retcode+") msg: "+msg+" data: " + parsedToken);
		...
		else	/// Authentication start
		{
			...
			String URL = ConfigUtils.get("URLAuthorize");
			String response_type = "code";
			String client_id =  ConfigUtils.get("OAUth_client_id");
			String redirect_uri = request.getRequestURL().toString();	// This servlet URL
			String scope = ConfigUtils.get("OAuth_scope");
			String state = UUID.randomUUID().toString().replaceAll("-", "");	// Generated value
			String nonce = UUID.randomUUID().toString().replaceAll("-", "");	// Generated value for remote server
			String urlQuery = String.format("%s?response_type=%s&client_id=%s&redirect_uri=%s&scope=%s&state=%s&nonce=%s",
